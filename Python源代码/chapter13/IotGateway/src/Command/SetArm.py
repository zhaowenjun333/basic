...
class CSetArm(CBaseCommand):
    ...
    def Run(self):
        with self.protocol.lockCmd:
            ...
            with SBDB.session_scope() as session :
                ...
                try:
                    apartment=session.query(SBDB_ORM.Apartment).filter(SBDB_ORM.Apartment.id==apartment_id).first()
    ...
    def PushToClients(self,session,time_):
        ...
        listIOS=[]
        language=0
        for client in session.query(SBDB_ORM.Client).filter(and_(SBDB_ORM.Client.account_id==self.protocol.account_id,SBDB_ORM.Client.os==BaseCommand.PV_OS_IOS)):
        ...
        if len(listIOS)>0:
            threading.Thread(target=Util.push_ios,args=(listIOS, "notification", message)).start()
