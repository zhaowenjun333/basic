...
class CBaseControl(CBaseCommand):
    ...
    def __init__(self,data=None,protocol=None):
        ...
        self.dictWaitingRelayerControls={} #the map of relayer_id-->array of CDeviceCmd
            ...
            try:
                for dc in listDC:
                    ...
                    if not self.internalMessage:
                        for s, in session.query(SBDB_ORM.Apartment_Relayer.relayer_id).join(SBDB_ORM.Apartment).join(SBDB_ORM.ApartmentDevice).join(SBDB_ORM.Device).filter(and_(SBDB_ORM.Apartment.account_id==self.account_id,SBDB_ORM.Device.uni_code==dc.dev_code)).all():
                            print(("kkkkkkkkkkkk", s, self.account_id, dc.dev_code))
                            if s in self.dictWaitingRelayerControls:
                                self.dictWaitingRelayerControls[s].append(dc)
                            else:
                                self.dictWaitingRelayerControls[s]=[dc,]
                    else:
                        relayer_id=self.internalMessage.destId
                        if relayer_id in self.dictWaitingRelayerControls:
                            self.dictWaitingRelayerControls[relayer_id].append(dc)
                        else:
                            self.dictWaitingRelayerControls[relayer_id]=[dc,]
    ...
    def SendBatch(self):
        for relayer_id in list(self.dictWaitingRelayerControls.keys()):
            ...
            if self.internalMessage:
                with self.protocol.factory.lockDict:
                    sb_protocol=self.protocol.factory.dictRelayer.get(relayer_id)
            ...
            if not bSend:
                self.dictFinishedRelayerControls[relayer_id]=self.dictSendingRelayerControls.pop(relayer_id)
                continue
            break
    ...
    def FinishOne(self,relayer_id,control_device,respond):
        with self.lock:
            ...
            deviceCmds=None
            for key in list(self.dictSendingRelayerControls.keys()):
                if key==relayer_id:
                                ...
                                with SBDB.session_scope() as session :
                                    ...
                                    if len(listRelayerId)>0:
                                        ...
                                        for s in listRelayerId:
                                            print(("AAAAAAAAAAAAAAAAAAAA", s))
                                            if s in self.dictWaitingRelayerControls:
                                                self.dictWaitingRelayerControls[s].append(deviceCmd)
                                            else:
                                                self.dictWaitingRelayerControls[s]=[deviceCmd,]
    ...
    def CheckFinished(self):
        if len(list(self.dictSendingRelayerControls.keys()))>0:
            for relayer_id in list(self.dictSendingRelayerControls.keys()):
                ...
                self.dictFinishedRelayerControls[relayer_id]=self.dictSendingRelayerControls.pop(relayer_id)
        if len(list(self.dictWaitingRelayerControls.keys()))<=0:
            return True
        else:
            self.SendBatch()
...
#         if self.protocol is not None and self.protocol.role ==BaseCommand.PV_ROLE_HUMAN:
        if self.protocol is not None:
            ...
            for relayer_id in list(self.dictFinishedRelayerControls.keys()):
    ...
    def Send(self):
        ...
        while True:
            with self.protocol.cond_dictControlling:
                if len(list(self.protocol.dictControlling.keys()))>maxcmd_flow_control:
                    ...
                    self.timer=reactor.callLater(Config.timeout_relayer_control,self.timeout)
                    ...
                    if self.requireCommand is not None and self.requireCommand.internalMessage is None:
                        interMessage=InternalMessage.CInternalMessage()
                        interMessage.SetParam(InternalMessage.TTYPE_GATEWAY,self.relayer_id,0,InternalMessage.OPER_REQUEST,"",
