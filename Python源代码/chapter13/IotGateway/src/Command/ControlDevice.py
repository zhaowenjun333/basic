...
class CControlDevice(CBaseControl):
    ...
    def initDictRelayerControls(self):
        ...
        self.initByDeviceCmdList([CDeviceCmd(dev_model,dev_code, dev_key_seq, dev_value),])
        ...
        with self.protocol.lockCmd:
            ...
            with self.protocol.cond_dictControlling:
                ...
                if request is not None: 
                    logging.debug("call self.protocol.cond_dictControlling.notify() due to CControlDeviceResp in protocol %d",id(self.protocol.transport))
                    if request.timer!=None:    
                        request.timer.cancel()
                        request.timer=None
                    self.protocol.cond_dictControlling.notify()
                else:
                    logging.debug("fail to self.protocol.dictControlling.pop(%d) due to CControlDeviceResp in protocol %d",self.command_seq,id(self.protocol.transport))
