...
class CBaseCommand(object):
    ...
    def __init__(self,data=None,protocol=None):
        ...
        if data is not None:     
            self.data=data
            self.command_len,self.command_id,self.command_status,self.command_seq=struct.unpack("!4I",data[:CBaseCommand.HEAD_LEN])
    ...
    def Authorized(self):
        return 'role' in dir(self.protocol) and self.protocol.role is not None and len(self.protocol.role)>0
...
#         max_block=200
#         while p<len_data:
#             if len_data-p>max_block:
#                 #self.protocol.transport.write(self.data[p:p+max_block])
#                 reactor.callFromThread(self.protocol.transport.write,self.data[p:p+max_block])
#                 p=p+max_block
#             else:
#                 #self.protocol.transport.write(self.data[p:])
#                 reactor.callFromThread(self.protocol.transport.write,self.data[p:])
#                 p=len_data
#             if p<len_data:
#                 time.sleep(0.2)
    ...
    def __str__(self, *args, **kwargs):
        return "<%s(%d,%d,%d,%d,%s>" % (self.__class__,self.command_len,self.command_id,self.command_status,self.command_seq,json.dumps(self.body))
    ...
    def SetErrorCode(self,command_status,error_string=None):
        ...
        if command_status !=CS_OK:
            if error_string is None:
                error_string=dictErrorString.get(command_status,"unknown error")
...
class CBaseRespCommand(CBaseCommand):
    ...
    def Send(self,internalMessage=None):
        ...
        if len(self.body)>0:
            body_string=json.dumps(self.body)
        self.command_len=CBaseCommand.HEAD_LEN+len(body_string)
#         self.data=struct.pack("!4I",CBaseCommand.HEAD_LEN+len(body_string),self.command_id,self.command_status,self.request.command_seq)
        self.data=struct.pack("!4I",self.command_len,self.command_id,self.command_status,self.request.command_seq)
        self.data=self.data+body_string
...
class CMesscodeCommand(CBaseCommand):
    ...
    def Run(self):
        ...
        logging.error("MesscodeCommand in transport %d : %s",id(self.protocol.transport),Util.asscii_string(self.data)) 
