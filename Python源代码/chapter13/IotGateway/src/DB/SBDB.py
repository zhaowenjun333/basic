...
def GetRelayerIdForcely(relayer_code):
    with session_scope() as session :
        try:
            relayer=session.query(SBDB_ORM.Relayer).filter_by(uni_code=relayer_code).with_lockmode('update').first()
            if relayer is None:
...
def GetRelayerIDsByAccountId(account_id,session=None):
    ...
    if session is None: 
        ...
        release=True
    RelayerIDs=[]
    for relayerId, in session.query(SBDB_ORM.Apartment_Relayer.relayer_id).join(SBDB_ORM.Apartment).join(SBDB_ORM.Account).filter(SBDB_ORM.Account.id==account_id).all():
...
def IncreaseVersions(session,relayer_id,apartment_id):
    if relayer_id==0:
        ...
        apartments=session.query(SBDB_ORM.Apartment).with_lockmode('update').join(SBDB_ORM.Apartment_Relayer).filter(SBDB_ORM.Apartment_Relayer.relayer_id==relayer_id).all()
...
def GetRelayeresByAccountId(accountId):
    with session_scope() as session :
        listRelayeres=[]
        for apartment_relayer in session.query(SBDB_ORM.Apartment_Relayer).join(SBDB_ORM.Apartment).filter(SBDB_ORM.Apartment.account_id==accountId).all():
