...
def SendAlarmBySMS(sms_template, apartment_name, alarm_type, listPhone,device_name):
    ...
    sms_template= urllib.parse.quote(sms_template.encode('gbk'))
...
def SendAndSave(sms_template, apartment, alarm_type, listPhone,device_name):
    ...
    with mutex:
        ...
        message,result,error=SendAlarmBySMS(sms_template,apartment.account.user_name,alarm_type,listPhone,device_name)
...
if __name__ == '__main__':
    result = SendAlarmBySMS('Please call 119', '101-102', 'fire alarm', ['18901688801', '18930519689'])
    if result == True:
