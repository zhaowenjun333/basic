...
asscii_string = lambda s: ''.join(["%02X " % ord(c) for c in s])
...
def SendSMSByZA(mobile_phone, content):
    q_value = '&mobile=%s&content=%s&sendTime=&extno='% (urllib.parse.quote(mobile_phone.encode('utf-8')), urllib.parse.quote(content.encode('utf-8')))
