...
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++业务判断+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/		    
	    	...
	    	/*获取编码*/
    		 ...
    		 for(var i=0;i<ak("blockOrNot").data.length;i++){
    			 if(ak("blockOrNot").data[i].text==params.blockOrNot){
    				 blockOrNotcode=ak("blockOrNot").data[i].id;
    				 ak("blockOrNot").setValue(blockOrNotcode);
    			 }
    		 }
    		 ...
    		 ak("blockCase").setValue(params.blockCase);
    		 ak("attachment").setValue(params.attachment);
//    		 _initAllDrop();
    		//分页组件
    		 var datajson={};
...
//    			 myak.setPageIndex(e);//跳转到对应页
		    ...
		    if(me.type == "编辑"){
		    	if(params.status==1 || params.status ==2){
		    		$(".searchPic").on("click",function(){//跟踪信息
		    			var newRow = {};
		    	...
		    	if(params.status==3 || params.status ==4 ){
		    		 ak("blockOrNot").readOnly=true;
		    		 ak("blockCase").readOnly=true;
				...
				$(".words").click(function(){ 
						if(count==1){
							...
							if(data.blockCase.length> 50){
								showtips("极闭锁动作情况最大长度50,请重新填写","info");
								return;
							}
							...
							myak.send('/fault/update','GET',data,true,function(text){ 
								showtips("更新成功");
...
//										插入应急抢修一条记录；
										...
										qxmk.stationOrLineId=ak("stationOrLineName").getValue();
										qxmk.deviceId=ak("deviceName").getValue();
										qxmk.organizationId=ak("organization").getValue();  
	...
	function _fileUpLoad(id){
		var iframe = null;
		 var win = ak.open({
		           title: '附件上传',
		//           url:"http://10.144.15.216:9091/upload.html?url=/user/mcsas/faultTrip/"+id+"/&readOnly=false",
	...
	/*function _initAllDrop() {
		gridRiskID=ak("attachment").getValue();
		if(gridRiskID =='' || gridRiskID == null){
...
//					ak("voltageLevel").setValue(stationTree.getNode(stationTree.getSelectedNode().pid).text);
...
//				换相失败，直流再启动，直流闭锁
		...
		ak('blockOrNot').load(akapp.appname+'/faultTrip_edit_new/getblockOrNot');// 是否 
