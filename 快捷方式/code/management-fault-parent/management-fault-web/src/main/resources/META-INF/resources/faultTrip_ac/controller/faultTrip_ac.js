...
//			赋值 这三个暂时不传值，
	    	...
	    	/*获取编码*/
    		 for(var i=0;i<ak("sendSuccessOrNot").data.length;i++){
	    		 if(ak("sendSuccessOrNot").data[i].text==params.sendSuccessOrNot){
	    			 sendSuccessOrNotcode=ak("sendSuccessOrNot").data[i].id;
	    			 ak("sendSuccessOrNot").setValue(sendSuccessOrNotcode) ; 
	    		 }
	    	 }
    		 ...
    		 for(var i=0;i<ak("stationFaultOrNot").data.length;i++){
    			 if(ak("stationFaultOrNot").data[i].text==params.stationFaultOrNot){
    				 stationFaultOrNotcode=ak("stationFaultOrNot").data[i].id;
    				  ak("stationFaultOrNot").setValue(stationFaultOrNotcode);
    			 }
    		 }
    		 for(var i=0;i<ak("reclosingAction").data.length;i++){
    			 if(ak("reclosingAction").data[i].text==params.reclosingAction){
    				 reclosingActioncode=ak("reclosingAction").data[i].id;
    				 ak("reclosingAction").setValue(reclosingActioncode);
    			 }
    		 }
    		 ak("recoveryDate").setValue(params.recoveryDate); 
    		 ak("faultLocator").setValue(params.faultLocator);
    		 ak("faultReason").setValue(params.faultReason);
    		 ak("protection").setValue(params.protection);
    		 ak("managementDetail").setValue(params.managementDetail);
    		 ak("faultDetail").setValue(params.faultDetail);
    		 var datajson={};
    	...
    	//分页组件
    		...
    		/* myak.send('/fault/queryWarnTrackStatusResult','GET',datajson,false,function(data){
			 	 dataResult = ak.decode(data);
			 	 if(dataResult !=null && dataResult.length>0){
			 		 me.WarnTrackLength=dataResult.length;//获取后台查询跟踪情况的表格长度
			 		 for(var i=1;i<=dataResult.length;i++){
			 		 }
			 	...
			 	//fillData(0,datagria.getPageSize(), dataResult, datagria );
     		});*/
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++业务判断+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/		    
		    //编辑的时候
		    if(me.type == "编辑"){
		    	...
		    	ak("faultPhaseId").readOnly=true;
		    	 if(params.status==3 || params.status ==4 ){
			    	 ...
			    	 ak("reclosingAction").readOnly=true;
			    	 ak("sendSuccessOrNot").readOnly=true;
			    	 ak("repairEndOrNot").readOnly=true;
			    	 ak("repairOrNot").readOnly=true;
			    	 ak("faultLocator").readOnly=true;
			    	 ak("recoveryDate").readOnly=true;
			    	 ak("faultReason").readOnly=true;
			    	 ak("protection").readOnly=true;
			    	 ak("managementDetail").readOnly=true;
			    	 ak("faultDetail").readOnly=true;
		 		...
		 		$(".words").click(function(){ 
						...
						if(count==1){
							var form = new ak.Form("#form");            
			 				...
			 				if(data.faultReason.length> 21){
								showtips("故障原因最大长度21,请重新填写","info");
								return;
							}
			 				if(data.protection.length> 50){
								showtips("保护动作情况最大长度50,请重新填写","info");
								return;
							}
			 				if(data.faultLocator.length> 50){
								showtips("故障测距最大长度50,请重新填写","info");
								return;
							}
			 				if(data.managementDetail.length> 50){
								showtips("处理详情最大长度50,请重新填写","info");
								return;
							}
							if(data.faultDetail.length> 250){
								showtips("故障详情最大长度250,请重新填写","info");
								return;
							}
			 				data.id=params.id;
...
//									返回值是一个json对象
						    ...
						    flgsomedata =ak("datatail").getSelecteds();//跟踪情况表格
						    var trackOrNotTable = datagria.getData(); // 跟踪情况
						    if(me.WarnTrackLength!=undefined){  //grid有数值 更新
		     ...
		     //查看的时候
		    if(me.type == "查看"){ 
		    	//把input属性设置为:allowInput="false",才可以禁用输入
		    	 ak("faultType").readOnly=true;
		    	 ak("faultCategory").readOnly=true;
		    	 ak("faultPhaseId").readOnly=true;
		    	 ak("faultTime").readOnly=true;
		...
		ak("repairOrNot").on('valuechanged',function(e){
		...
		ak("datatail").on("cellbeginedit",function(p){
			if(p.rowIndex <  me.WarnTrackLength){//获取后台查询跟踪情况的表格长度
				 p.cancel="true";
			}
	 	 }) ; 
		//选中事件
		...
		//切换前的选中对象p,获取的selects是之前的并不是当前的
...
//				换相失败，直流再启动，直流闭锁
		...
		ak('sendSuccessOrNot').load(akapp.appname+'/faultTrip_edit_new/getsendSuccessOrNot');// 强送成功
		ak('repairOrNot').load(akapp.appname+'/faultTrip_edit_new/getrepairOrNot');// 是否抢修
		ak('repairEndOrNot').load(akapp.appname+'/faultTrip_edit_new/getrepairEndOrNot');//是否抢修成功
		...
		ak('reclosingAction').load(akapp.appname+'/faultTrip_edit_new/getreclosingAction');// 重合闸动作情况
